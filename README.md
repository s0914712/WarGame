...
| **反潛護航 2031** `asw_escort_2031` | 補給艦進蘇澳港 · 100 min | 聲納方程式 + E21 測向/TMA/吊放聲納/混響/深度水文；護航突破潛艦封鎖 |

### 聚焦：反潛護航 2031（聲學對抗深度示範）

磐石號補給艦須由台灣東部深水區**穿越危險海域、安全抵達蘇澳港**；2 艘解放軍潛艦於蘇澳航道上潛伏——一艘**層內淺潛 40m**
（同層直達聲傳、較易偵獲）、一艘**層下深潛 200m**（跨層 +8dB 衰減、難偵獲）。藍方反潛群（巡防艦主動聲納 + 拖曳陣列 +
P-8 聲標屏幕 + S-70C 反潛直升機吊放聲納 + 獵殺潛艦）須護衛補給艦突破封鎖，靠**三角交會（持續 ~90s）/ TMA / 主動定位**
把「未定位」的測向接觸變成可開火的火控解（且僅得匿名標記，完整識別須升潛望鏡目視）。
場景開始前的 **BT 溫深剖面 + 海況 + 底質 + 水深** 會導出層深、會聚區、環境噪音與混響強度，直接左右各單位的偵測距離。

**勝負**：藍方 = 補給艦抵達蘇澳港；紅方 = 擊沉補給艦，或擊沉兩艘護衛艦（FFG 全滅）。
...
### 線上 2 人對戰（host-authoritative）

主選單「線上對戰」可藍 vs 紅各自一台裝置連線推演：
- **主機（藍）**：跑權威引擎、廣播戰場狀態；建立房間後把**房號**給對手。
- **客戶端（紅）**：輸入房號加入，鏡像主機狀態、回傳指令；時鐘由主機控制。
- 每方只能指揮自己陣營、各自套用戰場迷霧（POV 鎖定）；指令在主機端驗證擁有權。
- 傳輸走 **Supabase Realtime**（靜態部署即可用），需設定 `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY`。
- 加入連結：`?room=<房號>`。MVP 採信任模型（客戶端收完整狀態，僅渲染端套迷霧）；防作弊的「每方過濾狀態」、
  斷線重連、host 遷移、觀戰為後續強化項。
